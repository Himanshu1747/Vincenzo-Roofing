...
import { S as unescapeHTML, p as generateCspDigest, t as spreadAttributes, u as renderTemplate } from "./server_VgpkRdB2.mjs";
...
var ImmutableDataStore = class ImmutableDataStore {
	...
	*/
	static async fromModule() {
		try {
			const data = await import("./_astro_data-layer-content_D2m6tGEK.mjs");
...
function createGetCollection({ liveCollections }) {
	return async function getCollection(collection, filter) {
		...
		if (await store.hasCollection(collection)) {
			const { default: imageAssetMap } = await import("./content-assets_DyQ12uTg.mjs");
			const result = [];
