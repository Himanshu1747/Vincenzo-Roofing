...
// No caching — every call goes straight to WordPress, so edits appear
// immediately. If your site gets heavy traffic, you'll want to bring
// caching back later (I can help with a webhook-based version then).
export async function queryWordPress(query, variables = {}) {
  const endpoint =
    import.meta.env.WP_API_URL ||
    "https://staging2.vincenzoroofing.com/graphql";
  ...
  return fetchWithRetry(endpoint, body);
}