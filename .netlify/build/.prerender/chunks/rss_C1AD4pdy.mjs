...
import { Z as UnknownContentCollectionError, ct as isRemotePath, dt as removeBase, t as AstroError } from "./errors_CStDCjdR.mjs";
import { E as unescapeHTML, f as renderTemplate, h as generateCspDigest, t as spreadAttributes } from "./server_CKpIG7MF.mjs";
...
var ImmutableDataStore = class ImmutableDataStore {
	...
	*/
	static async fromModule() {
		try {
			const data = await import("./_astro_data-layer-content_CHNwYaJg.mjs");
...
function createGetCollection({ liveCollections }) {
	return async function getCollection(collection, filter) {
		...
		if (await store.hasCollection(collection)) {
			const { default: imageAssetMap } = await import("./content-assets_DYhOMTXN.mjs");
			const result = [];
